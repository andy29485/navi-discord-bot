...
class Emby:
  def __init__(self, bot):
    ...
    self.emby = embypy.Emby(self.conf['address'], **self.conf['auth'])
  ...
  async def _add(self, ctx, *, query):
    ...
    results = await loop.run_in_executor(None, self.emby.search, query)
    if not results:
...
def makeEmbed(item):
  ...
  em.set_image(item.primary_image_url)
